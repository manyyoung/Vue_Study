<template>
  <!-- 
    ...
    1. sorting
    2. 외부로부터 column명 받기.
    3. 외부로부터 정제된 데이터로 props 받아 처리
 -->
  <div>
    <div style="display: flex; justify-content: space-between">
      <div class="table_total">총 게시글 {{ totalCnt }}개</div>
    </div>
    <div class="table">
      ...
      <ul class="common_ul" v-for="(item, index) in showItems" :key="index">
      ...
      <ul v-if="showItems.length < 1" class="common_ul">
        <!-- 데이터 없음 정의 -->
...
export default {
  ...
  created() {
    ...
    // 총 게시글 개수 세팅
    this.totalCnt = this.items.length;
    console.log('props 값 items: ', this.items);
  ...
  methods: {
    ...
    // 정렬 메서드 (TODO: sorting을 추가할 수 있다.)
    // sortingItems: function (x, y) {
    //   return x.no < y.no ? 1 : x.no > y.no ? -1 : 0;
    // },
    setCurPage: function (curPage) {
      console.log('페이징 이동 ', curPage);
      if (curPage <= 0) {
        this.curPage = 1;
    ...
    setPagingList() {
      ...
      // this.$forceUpdate();
      console.log('start ', this.curPage * this.listRange);
      console.log('end', this.curPage * this.listRange + this.listRange);
      console.log('보여줄 리스트', this.showItems);
...
// 참조
// https://rgy0409.tistory.com/2970
// https://codepen.io/regic/pen/bEabEO
//  https://yoursyun.tistory.com/entry/ul-%ED%83%9C%EA%B7%B8%EB%A1%9C-%ED%85%8C%EC%9D%B4%EB%B8%94-%EC%B2%98%EB%9F%BC-%EC%82%AC%EC%9A%A9-%ED%95%98%EA%B8%B0-%EC%9C%84%ED%95%9C-style
// scss https://sass-lang.com/documentation/at-rules/use

// css 게임
// https://flukeout.github.io/
// https://studiomeal.com/archives/924
